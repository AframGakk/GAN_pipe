...
class GanService:
    def __init__(self, batch_size = 64, lrelu_alpha=0.2, adam_learning_rate=0.0002, adam_beta1=0.5):
    ...
    def train_old(self, epochs):
        ...
        for epoch in range(epochs):
            ...
            if epoch % 50 == 0:
                ...
                _modelRepo.saveDataToBucket('generator_{}'.format(self.title), self.generator.model)
                #_modelRepo.save_model_local('generator', self.generator.model)
                _modelRepo.saveDataToBucket('discriminator_{}'.format(self.title), self.generator.model)
                _modelRepo.saveDataToBucket('adverserial_{}'.format(self.title), self.generator.model)
    ...
    def plot_history_old(self, title, d1_hist, g_hist, a1_hist, a2_hist, bucket_save=False):
        ...
        if bucket_save:
            ...
            _modelRepo.saveFigureToBucket(filename)
        plt.close()
    ...
    def train(self, epochs=1000000):
        ...
        for epoch in range(epochs):
            ...
            if epoch % 50 == 0:
                ...
                #self.plot_losses(epoch, bucket_save=True)
                _modelRepo.saveDataToBucket('generator_{}_old'.format(self.title), self.generator.model)
                _modelRepo.saveDataToBucket('discriminator_{}_old'.format(self.title), self.generator.model)
                _modelRepo.saveDataToBucket('adverserial_{}_old'.format(self.title), self.generator.model)
                #first_sound = self.generate_sound(1)[0]
                #sf.write('./tmp/sample.wav', first_sound, 16000, subtype='PCM_16')
                #_modelRepo.saveSoundToBucket('./tmp/sample.wav', epoch)
